...
class AVLTree2<E extends Comparable<E>> extends BST2<E> {
	...
	public E find(double k, AVLTree2Node<E> node) {
		...
		if (k < 1 || k > rootSize())
			...
			if (node.left == null) {
				...
				if (k <= ((AVLTree2Node<E>) (node.left)).size)
					result = find(k, (AVLTree2Node<E>) node.left);
				else if (k == ((AVLTree2Node<E>) (node.left)).size + 1)
					result = node.element;
				else
					result = find((k - ((AVLTree2Node<E>) (node.left)).size - 1), (AVLTree2Node<E>) node.right);
			}
		}
		return result;
	}
	...
	private void updateSize(AVLTree2Node<E> node) {
		...
		if (node.left == null && node.right == null)
			...
			node.size = ((AVLTree2Node<E>) (node.right)).size + 1;
		else if (node.right == null)
			node.size = ((AVLTree2Node<E>) (node.left)).size + 1;
		else
			node.size = ((AVLTree2Node<E>) (node.right)).size + ((AVLTree2Node<E>) (node.left)).size + 1;
	}
...
Enter 15 numbers: 1 2 77 94 55 34 48 95 23 44 27 11 78 87 59
Enter k: 5
The 5th smallest number is 27.0
